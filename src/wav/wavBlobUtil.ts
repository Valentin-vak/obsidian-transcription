function _writeStringToArray(aString: string, targetArray: any, offset: number) {
  for (let i = 0; i < aString.length; ++i)
      targetArray[offset + i] = aString.charCodeAt(i);
}

function _writeInt16ToArray(aNumber: number, targetArray: any, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255;          // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255;   // byte 2
}

function _writeInt32ToArray(aNumber: number, targetArray: any, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255;          // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255;   // byte 2
...
async function _getAudioBuffer(blobData: any, contextOptions = undefined) {
  let blob = blobData;
...
export async function getWaveBlob(
  blobData: any, as32BitFloat: any, contextOptions = undefined
) {
